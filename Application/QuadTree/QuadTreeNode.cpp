...
void QuadTreeNode::DrawBounds()
{
    ...
    {
        // Top
        Engine::Get().WriteToBuffer({ bounds.GetX() + ix, bounds.GetY() }, hpbarChar, Color::Red, 0); 
        // Bottom
        Engine::Get().WriteToBuffer({ bounds.GetX() + ix, bounds.MaxY() - 1 }, hpbarChar, Color::Red, 0); 
	}

    for (int iy = 0; iy < bounds.GetHeight(); ++iy)
    {
        // Left
        Engine::Get().WriteToBuffer({ bounds.GetX(), bounds.GetY() + iy }, hpbarChar, Color::Red, 0);
        // Right
        Engine::Get().WriteToBuffer({ bounds.MaxX() - 1, bounds.GetY() + iy }, hpbarChar, Color::Red, 0);
    }
